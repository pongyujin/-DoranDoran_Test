...
def create_obstacle_map(lat_grid, lon_grid, obstacles, moving_obstacles, ship_lat, ship_lon, sensing_radius,
                        obstacle_radius=0.5, avoidance_cost=5):
    ...
    for moving_obstacle in moving_obstacles:
        ...
        if distance_to_ship <= sensing_radius:
            ...
            left_angle = (direction - 90) % 360
            rad = np.radians(left_angle)
...
def get_current_heading():
    ...
    # 실시간 real-time
        ...
        lat, lon, speed, current_heading=get_gps_data()
        #current_heading = 0 # 실시간 GPS로 부터 받아온다

    return current_heading

# 모터와 방향타 제어 함수
def control_motors_and_rudder(heading_error, motor_speed):
    if simulation_mode:
        pass
    else: # 출력은 90도 + heading_error # 라즈베리파이에서 PID 제어를 하고 결과를 모터와 방향타에 출력한다
        # 선박 속도 설정 (예: 오차가 크면 속도 감소)
        if abs(heading_error) > 45:
            motor_speed = 15  # 세팅속도의 1/2
        else:
            motor_speed = 30  # 세팅속도 30km/h
        heading = 90 + heading_error # 서보모터에 각도값를 세팅한다
        if heading >= 125: heading = 125
        if heading <= 55: heading = 55

        print(f"모터 속도: {motor_speed}, 방향타 각도: {heading-90}")
        #heading을 서보모터로 출력한다
...
def adjust_course(ship_lat, ship_lon, route_lats, route_lons):
    ...
    # 모터와 방향타 제어
    control_motors_and_rudder(heading_error, ship_speed)

# 실제 GPS 데이터를 받아오는 함수
def get_gps_data():
    # 실제 GPS 데이터(위도,경도,속도,방향)를 받아오는 코드 구현 필요
    # 실시간 real-time

    current_lat, current_lon, speed, direction =  34.5, 128.73, 0, 0
    gps_lat = current_lat  # 시뮬레이션에서는 현재 위치 사용
    gps_lon = current_lon
    return gps_lat, gps_lon, speed, direction

def on_ship_control(event):
    global ship_speed
    if simulation_mode:
        if event.key == '1':
            ship_speed = 30
    ...
    # 실시간 real-time
        ...
        try:
            ...
            if event.key == '1':
                ...
                # 모터와 방향타 제어
                ship_speed = 30
                control_motors_and_rudder(0, ship_speed)
            elif event.key == '0':
                ship_speed = 0
                control_motors_and_rudder(0, ship_speed)
                print("선박 정지: 0 km/h")
...
# 실시간 real-time
    ...
    initial_lat, initial_lon, initial_speed, initial_direction = get_gps_data()
    # 관제 화면에서 입력한 목적지를 입력한다
    destination_lat, destination_lon = 35.22, 128.90
    ship_speed = initial_speed = 0  # 실제 속도는 이후에 계산됨 #
...
sensing_radius = 3  # km
obstacle_radius = 0.5  # km  # 장애물 반경 설정
...
# 실시간 real-time
    ...
    # 이동 장애물 리스트 (30곳) # 실시간 이동장애물은  'direction': 0, 'speed': 0
    moving_obstacles = [
        {'lat': 34.85, 'lon': 128.85, 'direction': 0, 'speed': 0},
        {'lat': 35.0, 'lon': 129.0, 'direction': 0, 'speed': 0},
        {'lat': 34.95, 'lon': 129.25, 'direction': 0, 'speed': 0},
        {'lat': 34.70, 'lon': 128.80, 'direction': 0, 'speed': 0},
        {'lat': 34.75, 'lon': 129.00, 'direction': 0, 'speed': 0},
        {'lat': 34.90, 'lon': 129.10, 'direction': 0, 'speed': 0},
        {'lat': 34.60, 'lon': 128.75, 'direction': 0, 'speed': 0},
        {'lat': 34.85, 'lon': 128.95, 'direction': 0, 'speed': 0},
        {'lat': 35.05, 'lon': 128.90, 'direction': 0, 'speed': 0},
        {'lat': 34.95, 'lon': 129.10, 'direction': 0, 'speed': 0},
        {'lat': 34.65, 'lon': 128.85, 'direction': 0, 'speed': 0},
        {'lat': 34.80, 'lon': 128.90, 'direction': 0, 'speed': 0},
        {'lat': 35.10, 'lon': 129.05, 'direction': 0, 'speed': 0},
        {'lat': 34.75, 'lon': 128.75, 'direction': 0, 'speed': 0},
        {'lat': 34.88, 'lon': 129.05, 'direction': 0, 'speed': 0},
        {'lat': 34.92, 'lon': 129.20, 'direction': 0, 'speed': 0},
        {'lat': 35.12, 'lon': 129.00, 'direction': 0, 'speed': 0},
        {'lat': 34.82, 'lon': 128.88, 'direction': 0, 'speed': 0},
        {'lat': 34.78, 'lon': 129.15, 'direction': 0, 'speed': 0},
        {'lat': 34.67, 'lon': 128.78, 'direction': 0, 'speed': 0},
        {'lat': 34.85, 'lon': 128.80, 'direction': 0, 'speed': 0},
        {'lat': 34.90, 'lon': 128.95, 'direction': 0, 'speed': 0},
        {'lat': 35.05, 'lon': 129.15, 'direction': 0, 'speed': 0},
        {'lat': 34.65, 'lon': 128.70, 'direction': 0, 'speed': 0},
        {'lat': 34.80, 'lon': 129.05, 'direction': 0, 'speed': 0},
        {'lat': 34.95, 'lon': 129.05, 'direction': 0, 'speed': 0},
        {'lat': 34.55, 'lon': 128.80, 'direction': 0, 'speed': 0},
        {'lat': 34.90, 'lon': 128.85, 'direction': 0, 'speed': 0},
        {'lat': 35.00, 'lon': 128.95, 'direction': 0, 'speed': 0},
        {'lat': 34.85, 'lon': 129.00, 'direction': 0, 'speed': 0},
    ]
...
def update(frame):
    ...
    if simulation_mode:
        # 무인 선박의 이동 거리 업데이트
        distance_step = ship_speed * (time_step / 60)  # km // 1분 /60분    1분당 간거리
    ...
    if current_waypoint_index < len(waypoints):
        ...
        if distance_to_waypoint <= 1.0: ship_speed = 10
        arrival_threshold = max(0.1, min(0.5, ship_speed * 0.04))  # 100m~500m
        print(f"arrival_threshold: {arrival_threshold:.1f}")
        if distance_to_waypoint < arrival_threshold:  # 100m~500m 이내로 접근하면 도달한 것으로 간주
            # 경유점에서 정지
            stop_ship()
...
# 키를 눌러 운항 재개시킨다
plt.connect('key_press_event', on_ship_control)
# 애니메이션 실행
