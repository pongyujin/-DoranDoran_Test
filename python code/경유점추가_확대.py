...
# 축 범위 설정 함수 추가
def set_axis_limits(ax, center_lat, center_lon, zoom=False):
    global lat_min_plot, lat_max_plot, lon_min_plot, lon_max_plot

    if zoom:
        lat_range = (lat_max_plot - lat_min_plot) / zoom_factor
        lon_range = (lon_max_plot - lon_min_plot) / zoom_factor
        ax.set_xlim(center_lon - lon_range / 2, center_lon + lon_range / 2)
...
def update(frame):
    ...
    global full_route_lats, full_route_lons, is_zoomed
    ...
    if current_waypoint_index < len(waypoints):
        ...
        if distance_to_waypoint < 0.5:  # 500m 이내로 접근하면 도달한 것으로 간주
            ...
            print(f"경유점과의 거리는 {distance_to_waypoint}km 입니다")
    ...
    # 10초 후 확대 상태로 전환
    if current_time >= zoom_after and not is_zoomed:
        is_zoomed = True
        print("확대 모드로 전환합니다.")

    # 축 범위 설정
    set_axis_limits(ax, current_lat, current_lon, is_zoomed)
    ...
    for i, (wp_lat, wp_lon) in enumerate(waypoints[:-1], 1):
        if i >= current_waypoint_index:
            ax.scatter(wp_lon, wp_lat, marker='s', color='orange', s=100,
                       label=f'경유점 {i}' if i == current_waypoint_index else "")
